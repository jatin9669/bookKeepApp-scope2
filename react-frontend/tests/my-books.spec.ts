...
test.describe('Book Library Application', () => {
  test('should load books correctly for users', async ({ page }) => {
    ...
    await expect(page.getByRole('heading', { name: 'Books to return', exact: true })).toBeVisible();
    ...
    if (booksDoNotExist) {
      await expect(page.getByText('Get started by creating a new book.')).toBeVisible();
    } else {
